...
class Concentration {
    ...
    private(set) var cards = [Card]() // 空列表，列表对象为Card()
        ...
        get {// 每当用到该变量 调用
            ...
            for index in cards.indices {
                if cards[index].isFaceUp {
                    if foundIndex == nil {
                        foundIndex = index //找到第一张卡片
                    } else {
                        return nil //如果找到第二张卡片 返回nil
                    }
                }
            }
            return foundIndex //找到仅一张卡片返回该卡片index
        }
        set { //每当对该变量赋值 调用 内部变量newValue
            for index in cards.indices {
